...
# Copyright (c) 2025 - Jordan Kanter. All rights reserved
...
_nummap = {
    INT: lambda s: locale.atoi(s),
    LONGINT: lambda s: locale.atol(s),
    FLOAT: lambda s: locale.atof(s)
    }
...
class NumberObject(Evalable):
    ...
    def eval(self, env, args=None): # not returning self.v here causes a weird comparison issue, need to figure that oout.
        return self.v
...
class LogicObject(Evalable):
    ...
    def __repr__(self):
        if self.v == 1.0 or self.v == 1:
            return '*true*'
        elif self.v == 0.0 or self.v == 0:
            return '*false*'
        else:
...
class Reader(object):
    ...
    def get_token(self):
        ...
        while self.i < self.len and self.s[self.i] in WS:
            self.i = self.i + 1
        ...
        if self.s[self.i] == '#':
            ...
            # for vectors, I am just adding some logic to turn it into a standard
            # s-expression, because we don't support lists
            # ideally, this we should support a vector type
            self.i = self.i + 2
            # if it's just a pound followed by an s-expression,
            # pop the lookahead off the token stack and continue
            # as if it is just a regular s-expression (we do not support a vector type yet)
            if self.s[self.i - 1] == '(':
                self.i = self.i - 2
                self.s = self.s.replace('#',"'") # just remove the hash
            else:
                return self.pounds[self.s[self.i - 1]](self.get_token())
        if self.s[self.i] in SPECIAL:
            self.i = self.i + 1
            #print "\tSPECIAL:", self.s[self.i - 1], "\t", self.i, self.len
            return self.s[self.i - 1]
        elif self.s[self.i] == '"':
            # Parse a string.
            s = ""
            self.i = self.i + 1
            while self.s[self.i] != '"' and self.i < self.len:
                if self.s[self.i] == '\\':
                    self.i = self.i + 1
                    spchar = self.s[self.i]
                    if spchar == "n":
                        s = s + "\n"
                    elif spchar == "t":
                        s = s + "\t"
            ...
            while self.i < self.len - 1:
                if self.s[self.i] in SYNTAX:
                    break
                else:
                    tok = tok + self.s[self.i]
                    self.i = self.i + 1

            if not self.s[self.i] in SYNTAX:
                tok = tok + self.s[self.i]
                self.i = self.i + 1
...
class Lisper(object):
    ...
    def __init__(self, iostreams=(sys.stdin, sys.stdout, sys.stderr)):
        ...
        self.intern('last',FunctionObject(self.do_last))
        self.intern('car', FunctionObject(self.do_first))
        self.intern('rplaca', FunctionObject(self.do_rplaca))
...
# macros/syntax
    ...
    def do_last(self, env, args):
        return args[0].eval(env).last()

    def do_rplaca(self, env, args):
        return args[0].eval(env).rplaca(args[1].eval(env))
